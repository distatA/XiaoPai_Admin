            ...
            <el-form class="form" ref="form" :model="form" label-width="180px" size="small">
                ...
                <el-form-item label="详情轮播图">
                    <upload
                        ...
                        :limit="2"
                        url="/image_school"
                        :file="JSON.stringify(form.xiangqing)"
                    ></upload>
                </el-form-item>
...
export default {
    ...
    created() {
        ...
        this.loadOptions();
        this.$post("/admin/school/getone", { id: this.form.id }).then(res => {
            this.form = res.info;
            this.form.disciplinary_level = JSON.parse(
                this.form.disciplinary_level
            );
            this.form.feature_label = JSON.parse(this.form.feature_label);
            this.form.institution_type = JSON.parse(this.form.institution_type);
            this.form.description = res.info_data.description;
            this.form.scholarships = res.info_data.scholarships;
            this.form.teacher_strength = res.info_data.teacher_strength;
            this.form.city_identity = res.info_data.city_identity;
        });
    },
    methods: {
        loadOptions() {
        ...
        add() {
            if (typeof this.form.city != "number") {
            ...
            this.$post("/admin/school/edit", this.form).then(res => {
                if (res.code === 1) {
                    this.$message({
                        showClose: true,
                        message: res.msg,
