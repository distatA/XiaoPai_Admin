<template>
    <el-main>
        ...
        <el-table border :data="policyList">
            ...
            <el-table-column prop="depart_name" label="院系名称" align="center"></el-table-column>
            <el-table-column prop="name" label="专业名称" align="center"></el-table-column>
            <el-table-column prop="remark" label="备注" align="center"></el-table-column>
            <el-table-column label="操作" width="250" align="center">
                <template slot-scope="scope">
                    <el-button @click="jump(scope.row)" type="primary" size="mini">查看分数</el-button>
        ...
        >
            <el-form :model="form" label-width="100px" size="small">
                <el-form-item label="所属院系">
                    <el-select v-model="form.did" filterable placeholder="请选择院系">
                        <el-option
...
export default {
    ...
    methods: {
        loadOption() {
            this.$post("/admin/live/condition_school_college", {
        ...
        loadPolicyList(pages) {
            this.page = pages;
            this.$post("/admin/live/index_school_college", {
                sid: this.$route.query.sid,
                num: this.offset,
                page:this.page
            }).then(res => {
        ...
        handleAdd() {
            ...
            this.$post("/admin/live/add_school_college", this.form).then(
                res => {
                    if (res.code === 1) {
                        this.$message({
                            message: res.msg,
        ...
        edit(item) {
            this.$post("/admin/live/getone_school_college", {
                id: item.id
            }).then(res => {
                if (res.code === 1) {
                    this.form = res.info;
        ...
        handleEdit() {
            this.$post("/admin/live/edit_school_college", this.form).then(
                res => {
                    if (res.code === 1) {
                        this.$message({
                            message: res.msg,
        ...
        handleDelete(item) {
            this.$confirm("此操作将永久删除该专业, 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$post("/admin/live/delete_school_college", {
                        id: item.id
                    }).then(res => {
                        if (res.code === 1) {
                            this.$message({
