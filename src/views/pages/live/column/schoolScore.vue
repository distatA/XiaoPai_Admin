<template>
    <el-main>
        ...
        <el-table border :data="policyList">
            <el-table-column prop="school_name" label="学校名称" align="center" width="200"></el-table-column>
            <el-table-column prop="year" label="年份" align="center"></el-table-column>
            <el-table-column prop="type" label="类型" align="center"></el-table-column>
            <el-table-column prop="plan_num" label="计划数" align="center"></el-table-column>
            <el-table-column prop="need_num" label="录取数" align="center"></el-table-column>
        ...
        >
            <el-form :model="form" label-width="100px" size="small">
                <el-form-item label="分数类别">
                    <el-select v-model="form.type" placeholder="请选择分数类别">
                        <el-option
                ...
                <el-form-item label="计划人数">
                    <el-input v-model="form.plan_num" placeholder="请输入计划人数"></el-input>
                </el-form-item>
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" v-else @click="handleEdit" size="small">确 定</el-button>
            </span>
        </el-dialog>
    </el-main>
</template>
...
export default {
    ...
    methods: {
        loadOption() {
            this.$post("/admin/live/condition_school_score").then(res => {
                if (res.code === 1) {
                    this.typeList = res.score_type;
        ...
        loadPolicyList(pages) {
            this.page = pages;
            this.$post("/admin/live/index_school_score", {
        ...
        handleAdd() {
            ...
            this.$post("/admin/live/add_school_score", this.form).then(res => {
                if (res.code === 1) {
                    this.$message({
                        message: res.msg,
                        type: "success"
        ...
        edit(item) {
            this.$post("/admin/live/getone_school_score", { id: item.id }).then(
                res => {
                    if (res.code === 1) {
                        this.form = res.info;
                        this.isAdd = false;
        ...
        handleEdit() {
            this.$post("/admin/live/edit_school_score", this.form).then(res => {
                if (res.code === 1) {
                    this.$message({
                        message: res.msg,
                        type: "success"
        ...
        handleDelete(item) {
            this.$confirm("此操作将永久删除该分数, 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$post("/admin/live/delete_school_score", {
                        id: item.id
                    }).then(res => {
                        if (res.code === 1) {
                            this.$message({
