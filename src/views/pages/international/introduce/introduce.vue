<template>
    <el-main>
        <div class="flex">
            <div>
                <el-button
                    type="primary"
                    size="mini"
                    @click="handleJump('','international-modifyIntroduce','true')"
                >添加教学环境</el-button>
                <el-button
                    type="danger"
                    size="mini"
        ...
        <el-table border :data="introduceList" @selection-change="handleSelect">
            ...
            <el-table-column label="操作" width="200" align="center">
                <template slot-scope="scope">
                    <el-button
                        @click="handleJump(scope.row,'international-modifyIntroduce','false')"
                        type="primary"
                        size="mini"
                    >编辑</el-button>
...
export default {
    ...
    created() {
        this.loadIntroduceList();
    },
    methods: {
        //解决搜索之后，点击分页仍可以查询全部的bug
        getCurrentage(pages) {
            this.loadIntroduceList(pages);
        },
        //加载分类招生真题列表
        loadIntroduceList(pages, province) {
            this.page = pages;
            this.$post("/admin/abroad/introduce", {
        ...
        changeSort(item, value, dateSort) {
            this.$post("/admin/abroad/introduce_sort", {
                id: item.id,
                sort: value
            }).then(res => {});
        },
        ...
        handleDelete(item) {
            ...
            this.$confirm("此操作将永久删除学校, 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$post("/admin/abroad/introduce_del", {
