<template>
    <el-main>
        <div class="flex">
            <div>
                <el-button type="primary" size="mini" @click="add">添加搜索</el-button>
            ...
            <div class="flex">
                ...
                <el-button type="primary" size="mini" @click="loadSearchList(1,province)">搜索</el-button>
            </div>
        </div>

        <el-table border :data="searchList">
...
export default {
    ...
    methods: {
        loadOptions() {
            this.$post("/admin/classifie/condition_search").then(res => {
                if (res.code === 1) {
                    this.region = res.region; //地区搜索
                    this.typeList = res.search_type;
                }
            });
        },
        ...
        getCurrentage(pages) {
            console.log(pages);
            this.loadSearchList(pages, this.province);
        },
        //加载学校列表
        loadSearchList(pages, province) {
            this.page = pages;
            this.$post("/admin/classifie/index_search", {
                page: this.page,
                num: this.offset,
                province: province
            }).then(res => {
        ...
        handleAdd() {
            this.$post("/admin/classifie/add_search", this.form).then(res => {
                this.$message({
                    message: res.msg,
                    type: "success"
                });
        ...
        edit(item) {
            this.$post("/admin/classifie/getone_search", { id: item.id }).then(
                res => {
                    this.form = res.info;
                    this.dialogVisible = true;
                    this.isAdd = false;
        ...
        handleEdit() {
            this.$post("/admin/classifie/edit_search", this.form).then(res => {
                if (res.code === 1) {
                    this.$message({
                        message: res.msg,
                        type: "success"
        ...
        changeSort(item, value, dateSort) {
            this.$post("/admin/classifie/ajaxsort_search", {
                id: item.id,
                dataSort: dateSort,
                sort: value
            }).then(res => {});
        ...
        handleDelete(item) {
            ...
            this.$confirm("此操作将永久删除学校, 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$post("/admin/classifie/delete_search", {
