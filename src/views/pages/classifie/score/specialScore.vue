<template>
    <el-main>
        ...
        <el-table border :data="scoreList" @selection-change="handleSelect">
            ...
            <el-table-column prop="type" label="分数类型" align="center"></el-table-column>
        ...
        <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
            <el-form :model="form" ref="form" size="mini" :rules="formrules" label-width="100px">
                ...
                <el-form-item label="报名人数" prop="enroll_num">
...
export default {
    ...
    methods: {
        loadOptions() {
            this.$post("/admin/classifie/condition_school_scorefu").then(
                res => {
                    if (res.code === 1) {
        ...
        loadScoreList(pages) {
            this.page = pages;
            this.$post("/admin/classifie/index_school_scorefu", {
                page: this.page,
                num: this.offset,
        ...
        handleAdd() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    this.$post(
                        "/admin/classifie/add_school_scorefu",
                        this.form
                    ).then(res => {
                        this.$message({
                            message: res.msg,
        ...
        edit(item) {
            this.$post("/admin/classifie/getone_school_scorefu", {
                id: item.id
            }).then(res => {
                this.form = res.info;
                this.dialogVisible = true;
        ...
        handleEdit() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    this.form.sid = this.$route.query.sid;
                    this.$post(
                        "/admin/classifie/edit_school_scorefu",
                        this.form
                    ).then(res => {
                        if (res.code === 1) {
                            this.$message({
        ...
        handleDelete(item) {
            ...
            this.$confirm("此操作将永久删除分数, 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$post("/admin/classifie/delete_school_scorefu", {
