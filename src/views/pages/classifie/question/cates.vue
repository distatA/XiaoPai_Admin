<template>
    <el-main>
        <el-row class="add">
            <el-col :span="3">
                <el-button @click="add" type="primary" size="mini">添加真题分类</el-button>
            </el-col>
        </el-row>
        <el-table
            :data="cateList"
        ...
        <!-- 添加分类对话框 -->
        <el-dialog :title="isAdd?'添加真题分类':'编辑真题分类'" :visible.sync="dialogVisible" width="50%">
...
export default {
    ...
    methods: {
        // 获取政策分类列表
        loadCateList() {
            this.$post("/admin/classifie/index_question_cate").then(res => {
        ...
        //获取所有菜单列表
        loadOption() {
            this.$post("/admin/classifie/condition_question_cate").then(res => {
                if (res.code === 1) {
        ...
        // 确认添加菜单
        handleAdd() {
            this.$post("/admin/classifie/add_question_cate", this.form).then(
                res => {
                    if (res.code !== 1) {
        ...
        // 点击编辑弹框
        edit(item) {
            this.$post("/admin/classifie/getone_question_cate", {
                id: item.id
            }).then(res => {
                if (res.code === 1) {
                    this.form = res.info;
        ...
        //提交编辑确定按钮
        handleEdit() {
            this.$post("/admin/classifie/edit_question_cate", this.form).then(
                res => {
                    if (res.code === 1) {
                        this.form = {};
                        this.$message.success(res.msg);
        ...
        removeMenu(item) {
            ...
            this.$confirm("此操作将永久删除该分类, 是否继续?", "提示", {
                ...
                .then(() => {
                    this.$post("/admin/classifie/delete_question_cate", {
                        id: item.id
                    }).then(res => {
                        if (res.code === 1) {
                            this.$message({
